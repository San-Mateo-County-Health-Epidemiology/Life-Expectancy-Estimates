...
make_life_table <- function(data) {
  life_table <- data %>%
    mutate(int_width = case_when(age_cat == "0" ~ 1,
           ...
           prob_surv = case_when(age_cat != "90+" ~ (1-prob_dying)),
           num_alive_int = accumulate(.x = prob_surv,
                                     .init = 100000,
                                     .f = fun_alive_int),
           num_dying_int = case_when(age_cat == "90+" ~ num_alive_int,
                                     TRUE ~ num_alive_int - lead(num_alive_int)),
           pers_yrs_lived_int = case_when(age_cat == "90+" ~ num_alive_int/death_rate, 
           ...
           sample_var = case_when(age_cat == "90+" ~ (death_rate*(1-death_rate)/population),
                                  deaths == 0 ~ 0,
                                  TRUE ~ (prob_dying^2*(1-prob_dying)/deaths)),
           weighted_var = case_when(age_cat == "90+" ~ (num_alive_int^2)/death_rate^4*sample_var,
